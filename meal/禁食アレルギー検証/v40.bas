                ...
                For k = 0 To UBound(pocketNames)
                    ...
                    If foodName <> "" Then
                        ...
                        For w = LBound(allergyWords) To UBound(allergyWords)
                            ...
                            ' デバッグ: 橋章さん(=allergyContentに「牛・豚・固いもの」が含まれる人)だけログ
If InStr(allergyContent, "牛・豚・固いもの") > 0 Then
    Debug.Print "[橋章] word=[" & word & "] | food=" & foodNameForMatch
End If
                            If word <> "" Then
    ...
    ' デバッグ: れんこんを含むメニューだけログ出力
    If InStr(foodNameForMatch, "れんこん") > 0 Then
        Debug.Print "[直接マッチ] word=" & word & " | food=" & foodNameForMatch & " | content=" & allergyContent & " | FP=" & IsFalsePositiveByContext(word, allergyContent, foodNameForMatch, exclusionData, exclusionCount)
    End If
    ' ★ 除外ペアマスタによる偽陽性チェック
...
NextAllergy:
    ...
    If resultCount > 0 Then
        ...
        For i = 1 To resultCount
            ...
            numCourseStr = ""
            If numCourseDict.Exists(userName) Then
                numCourseStr = numCourseDict(userName)
            End If
...
Private Sub ProcessMenuSheetPattern1(ByVal sheetName As String, ByVal bentoType As String, _
    ...
    Do While r <= srcLastRow
        ...
        Else
            ...
            weekDataStartRow = 0
            For rr = weekHeaderRow + 1 To weekHeaderRow + 25
